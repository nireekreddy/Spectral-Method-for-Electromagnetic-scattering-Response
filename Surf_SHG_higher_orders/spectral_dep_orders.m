...
Hz = 1; % TE pol
lamvec=[];Delta1=[];Delta2=[];Delta3=[];Delta4=[];DD1=[];DD2=[];DD3=[];DD4=[];
lami=950e-9;lamf=1300e-9;nfac=1000;dlam=(lamf-lami)/nfac;
for uu=1:nfac;
...
%% SDm resonance factor
DD1=[DD1 detn(6)]; DD2=[DD2 detn(7)];
DD3=[DD3 detn(8)];DD4=[DD4 detn(9)];

%% higher order angluar modes
thth=pi/5;rm=a-0.000000001*a;
[xm,ym]=pol2cart(thth, rm);rp=a+0.000000001*a;
[xp,yp]=pol2cart(thth, rp);

secmod = CylinderGeometry;secmod.k = ksec; secmod.beta = beta;secmod.ep = ep; secmod.epi = episec;
secmod.mu = mu; secmod.mui = mui;secmod.a = a;
%% m=1
secmod.orders=[-1 1];
secmod.CmH(1)=sec.CmH(4);secmod.BmH(1)=sec.BmH(4);
secmod.CmH(2)=sec.CmH(6);secmod.BmH(2)=sec.BmH(6);
[~, Etsdm, ~, ~, ~, ~] = fieldpt(xm,ym, secmod);
[~, Etsdp, ~, ~, ~, ~] = fieldpt(xp,yp, secmod);
Etd=Etsdp-Etsdm;Etp=Etsdp+Etsdm;
Delta1= [Delta1 abs(Etd./Etp)];
%% m=2
secmod.orders=[-2 2];
secmod.CmH(1)=sec.CmH(3);secmod.BmH(1)=sec.BmH(3);
secmod.CmH(2)=sec.CmH(7);secmod.BmH(2)=sec.BmH(7);
[~, Etsdm, ~, ~, ~, ~] = fieldpt(xm,ym, secmod);
[~, Etsdp, ~, ~, ~, ~] = fieldpt(xp,yp, secmod);
Etd=Etsdp-Etsdm;Etp=Etsdp+Etsdm;
Delta2= [Delta2 abs(Etd./Etp)];
%% m=3
secmod.orders=[-3 3];
secmod.CmH(1)=sec.CmH(2);secmod.BmH(1)=sec.BmH(2);
secmod.CmH(2)=sec.CmH(8);secmod.BmH(2)=sec.BmH(8);
[~, Etsdm, ~, ~, ~, ~] = fieldpt(xm,ym, secmod);
[~, Etsdp, ~, ~, ~, ~] = fieldpt(xp,yp, secmod);
Etd=Etsdp-Etsdm;Etp=Etsdp+Etsdm;
Delta3= [Delta3 abs(Etd./Etp)];
%% m=4
secmod.orders=[-4 4];
secmod.CmH(1)=sec.CmH(1);secmod.BmH(1)=sec.BmH(1);
secmod.CmH(2)=sec.CmH(9);secmod.BmH(2)=sec.BmH(9);
[~, Etsdm, ~, ~, ~, ~] = fieldpt(xm,ym, secmod);
[~, Etsdp, ~, ~, ~, ~] = fieldpt(xp,yp, secmod);
Etd=Etsdp-Etsdm;Etp=Etsdp+Etsdm;
Delta4= [Delta4 abs(Etd./Etp)];

end
figure;
subplot(311)
plot(lamvec/1e-9,Delta1*100,'b',lamvec/1e-9,Delta2*100,'r',lamvec/1e-9,Delta3*100,'k',lamvec/1e-9,Delta4*100,'g')
set(gca,'fontsize',32)
set(gca,'TickLabelInterpreter','latex')
subplot(312)
plot(lamvec/1e-9,log10(DD1),'b',lamvec/1e-9,log10(DD2),'r',lamvec/1e-9,log10(DD3),'k',lamvec/1e-9,log10(DD4),'g')
set(gca,'fontsize',32)
set(gca,'TickLabelInterpreter','latex')
